...
func mockEnv(sender types.HumanAddress) typesv3.Env {
	return typesv3.Env{
...
func TestMigrate(t *testing.T) {
	...
	require.NoError(t, err)
	var qres typesv3.QueryResponse
	...
	data, _, err = Query(cache, id, query, &igasMeter, store, api, &querier, 100000000)
	require.NoError(t, err)
	var qres2 typesv3.QueryResponse
...
func TestMultipleInstances(t *testing.T) {
	...
	require.Equal(t, resp.Err, &typesv3.StdError{
		Unauthorized: &typesv3.Unauthorized{},
	})
...
func exec(t *testing.T, cache Cache, id []byte, signer types.HumanAddress, store KVStore, api *GoAPI, querier Querier, gasExpected uint64) typesv3.HandleResult {
	gasMeter := NewMockGasMeter(100000000)
	igasMeter := GasMeter(gasMeter)
...
func TestQuery(t *testing.T) {
	...
	require.NoError(t, err)
	var badResp typesv3.QueryResponse
	err = json.Unmarshal(data, &badResp)
	require.NoError(t, err)
	require.Equal(t, badResp.Err, &typesv3.StdError{
		ParseErr: &typesv3.ParseErr{
